...
athey.cv <- function(Y, S2, S1, X, S3){
  adj <- cbind(S3, S2, S1, X)
  cv.fit <- cv.glmnet(adj, Y, alpha=0)
  D <- function(X){return(predict(cv.fit, newx=X))}
  return(D)
}

athey.cv.elnet <- function(Y, S2, S1, X, S3){
  adj <- cbind(S3, S2, S1, X)
  cv.fit <- cv.glmnet(adj, Y)
  D <- function(X){return(predict(cv.fit, newx=X))}
  return(D)
}
...
for(idx in 1:length(data.list)){
  ...
  h1 <- outcome.ridge.bridge(obsY[obsA == 1], obsS2[obsA == 1,], obsS1[obsA == 1,], obsX[obsA == 1,], obsS3[obsA == 1,], 0.33)
  #estimate the outcome bridge function with A == 0
  h0 <- outcome.ridge.bridge(obsY[obsA == 0], obsS2[obsA == 0,], obsS1[obsA == 0,], obsX[obsA == 0,], obsS3[obsA == 0,], 0.33)
  ...
  q1 <- selection.ridge.bridge(obsS2[obsA == 1,], obsS1[obsA == 1,], obsX[obsA == 1,], obsS3[obsA == 1,],
                               expS2[expA == 1,], expS1[expA == 1,], expX[expA == 1,], expS3[expA == 1,], 0.33)
  q0 <- selection.ridge.bridge(obsS2[obsA == 0,], obsS1[obsA == 0,], obsX[obsA == 0,], obsS3[obsA == 0,],
                               expS2[expA == 0,], expS1[expA == 0,], expX[expA == 0,], expS3[expA == 0,], 0.33)
  ...
  #estimate the outcome bridge function with A == 1
  h1 <- outcome.ridge.bridge(obsY[obsA == 1], obsS2[obsA == 1,], obsS1[obsA == 1,], obsX[obsA == 1,], obsS3[obsA == 1,], 0.67)
  #estimate the outcome bridge function with A == 0
  h0 <- outcome.ridge.bridge(obsY[obsA == 0], obsS2[obsA == 0,], obsS1[obsA == 0,], obsX[obsA == 0,], obsS3[obsA == 0,], 0.67)
  tauridge2 <- mean(apply(cbind(expS3[expA == 1,], expS2[expA == 1,], expX[expA == 1,]), 1, h1))
  tauridge2 <- tauridge2 - mean(apply(cbind(expS3[expA == 0,], expS2[expA == 0,], expX[expA == 0,]), 1, h0))
  
  #selection bridge function
  q1 <- selection.ridge.bridge(obsS2[obsA == 1,], obsS1[obsA == 1,], obsX[obsA == 1,], obsS3[obsA == 1,],
                               expS2[expA == 1,], expS1[expA == 1,], expX[expA == 1,], expS3[expA == 1,], 0.67)
  q0 <- selection.ridge.bridge(obsS2[obsA == 0,], obsS1[obsA == 0,], obsX[obsA == 0,], obsS3[obsA == 0,],
                               expS2[expA == 0,], expS1[expA == 0,], expX[expA == 0,], expS3[expA == 0,], 0.67)
  obsq1 <- apply(cbind(obsS1[obsA == 1,], obsS2[obsA == 1,], obsX[obsA == 1,]), 1, q1)
  tauselridge2 <- mean(obsq1 * obsY[obsA == 1])
  obsq0 <- apply(cbind(obsS1[obsA == 0,], obsS2[obsA == 0,], obsX[obsA == 0,]), 1, q0)
  tauselridge2 <- tauselridge2 - mean(obsq0 * obsY[obsA == 0])
  
  #doubly robust estimation
  obsh1 <- apply(cbind(obsS3[obsA == 1,], obsS2[obsA == 1,], obsX[obsA == 1,]), 1, h1)
  obsq1 <- apply(cbind(obsS1[obsA == 1,], obsS2[obsA == 1,], obsX[obsA == 1,]), 1, q1)
  taudrridge2 <- tauridge2 + mean(obsq1 * (obsY[obsA == 1] - obsh1))
  obsh0 <- apply(cbind(obsS3[obsA == 0,], obsS2[obsA == 0,], obsX[obsA == 0,]), 1, h0)
  obsq0 <- apply(cbind(obsS1[obsA == 0,], obsS2[obsA == 0,], obsX[obsA == 0,]), 1, q0)
  taudrridge2 <- taudrridge2 - mean(obsq0 * (obsY[obsA == 0] - obsh0))
  ...
  #                                 "tautrue", "truetau"))
  res.ridge.list[[idx]] <- mget(c("tauridge1", "tauselridge1", "taudrridge1", "tauridge2", "tauselridge2", "taudrridge2"))
}
