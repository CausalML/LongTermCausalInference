...
getrmse <- function(listval, keyword){
  sqrt(mean(sapply(listval, function(res) {abs(res[[keyword]] - res$tautrue)^2})))
}

getab <- function(listval, keyword){
  abs(mean(sapply(listval, function(res) {res[[keyword]] - res$tautrue})))
}
...
resrmsetab <- matrix(nrow=totpair, ncol=totoffset)
resabtab <- matrix(nrow=totpair, ncol=totoffset)
resdigimaetab <- matrix(nrow=totpair, ncol=totoffset)
resdigimedtab <- matrix(nrow=totpair, ncol=totoffset)
resdigirmsetab <- matrix(nrow=totpair, ncol=totoffset)
resdigiabtab <- matrix(nrow=totpair, ncol=totoffset)
...
for(oidx in 1:length(offset.list)){
  ...
  for(idx in 1:totpair) {resdigirmsetab[idx, oidx] <- getrmse(pairs[[idx]][[1]], pairs[[idx]][[2]])}
  
  for(idx in 1:totpair) {resdigiabtab[idx, oidx] <- getab(pairs[[idx]][[1]], pairs[[idx]][[2]])}

  for(idx in 1:totpair) 
    {resmaetab[idx, oidx] <- formatting(resdigimaetab[idx, oidx], 3)}
  
  for(idx in 1:totpair) 
    {resmedtab[idx, oidx] <- formatting(resdigimedtab[idx, oidx], 3)}

  for(idx in 1:totpair) 
    {resrmsetab[idx, oidx] <- formatting(resdigirmsetab[idx, oidx], 3)}
  
  for(idx in 1:totpair) 
    {resabtab[idx, oidx] <- formatting(resdigiabtab[idx, oidx], 3)}
  
  
  print(paste(resmaetab[,oidx], collapse=" & "))
  print(paste(resmedtab[,oidx], collapse=" & "))
  print(paste(resrmsetab[,oidx], collapse=" & "))
  print(paste(resabtab[,oidx], collapse=" & "))
}
